...
public class MainRandom {
	...
	public static void main(String[] args) {
		...
		try {
			...
			for(int prob=0;prob<problemas;prob++){
				...
				imprimeDatos(prob,resolutor.getNumComensales(),timeStream);
				imprimeResultados(prob,sol,tF-tI,outputStream,timeStream);
	...
	private static void imprimeResultados(int p,Solucion s,long lasted,PrintStream out,PrintStream ts){
		out.println("Problema: "+p);
		out.println(s);
		out.println();
		s.printInfoNodos(ts, lasted);
		ts.println();
	}
	private static void imprimeDatos(int p,int c,PrintStream out){
